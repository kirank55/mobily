/**
 * cli/src/pty/node-pty.ts
 *
 * Thin wrapper around `node-pty` that presents a clean, typed surface for the
 * rest of the CLI. No `PtyBackend` interface is extracted here — the task says
 * "only if a second implementation materialises". That abstraction arrives in
 * Phase 5 when `TmuxBackend` is built.
 *
 * Public API
 * ----------
