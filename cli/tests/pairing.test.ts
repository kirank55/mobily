...
async function setupPairedSession(): Promise<{
  ...
  const res = await fetch(`http://localhost:${server.port}/.well-known/mobily/pair`, {
    ...
    body: JSON.stringify({ code, deviceId, publicKey: publicKeyPem }),
  });
...
describe('pairing flow end-to-end', () => {
  it(
    'pairs via HTTP, then connects via WS and streams PTY output',
    async () => {
  ...
  it(
    'HTTP pairing response includes tunnelUrl, stationName, and protocolVersion',
    async () => {
      ...
      const res = await fetch(`http://localhost:${server.port}/.well-known/mobily/pair`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code, deviceId: 'dev1', publicKey: publicKeyPem }),
      });
...
describe('pairing endpoint error cases', () => {
  it(
    'returns 403 for an invalid pairing code',
    async () => {
      const auth = new AuthManager('test-station');
      ...
      const res = await fetch(`http://localhost:${server.port}/.well-known/mobily/pair`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code: 'WRONG', deviceId: 'dev1', publicKey: publicKeyPem }),
      });
...
describe('reconnect after disconnect', () => {
  it(
    'a second WS connection with the same device succeeds and the PTY is still alive',
    async () => {
      const { server, privateKeyPem, deviceId } = await setupPairedSession();
